...
function parseEditorContent(element: HTMLElement): Paragraph[] {
  debugLog("ODT Export - Parsing editor content")
  debugLog("ODT Export - Editor innerHTML:", element.innerHTML.substring(0, 500))
  ...
  function getCurrentStyles(node: Node): Partial<TextRun> {
    ...
    while (current && current !== element) {
      if (current.nodeType === Node.ELEMENT_NODE) {
        ...
        if (tagName === "s" || tagName === "strike" || tagName === "del") styles.strikethrough = true
        if (computedStyle.textDecoration.includes("line-through")) styles.strikethrough = true

        if (tagName === "font" && el.getAttribute("size")) {
          styles.fontSize = el.getAttribute("size") || undefined
        }

        const inlineSize = el.style.fontSize
        if (inlineSize) {
          const pxMatch = inlineSize.match(/(\d+)px/)
          if (pxMatch) {
            const px = Number.parseInt(pxMatch[1])
            if (px <= 10) styles.fontSize = "1"
            else if (px <= 13) styles.fontSize = "2"
            else if (px <= 16) styles.fontSize = "3"
            else if (px <= 18) styles.fontSize = "4"
            else if (px <= 24) styles.fontSize = "5"
            else if (px <= 32) styles.fontSize = "6"
            else styles.fontSize = "7"
          }
        }
      }
  ...
  function finishParagraph() {
    if (currentParagraph.runs.length > 0) {
      ...
      debugLog("ODT Export - Finished paragraph:", JSON.stringify(currentParagraph))
  ...
  function processNode(node: Node, inheritedHeading?: "h1" | "h2" | "h3"): void {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (cleanText) {
        ...
        debugLog("ODT Export - Text node:", cleanText.substring(0, 30), "styles:", JSON.stringify(styles))
    ...
    if (textAlign === "center") currentAlignment = "center"
    else if (textAlign === "right") currentAlignment = "right"
    else if (textAlign === "justify") currentAlignment = "justify"
    else if (textAlign === "left") currentAlignment = "left"
    ...
    if (tagName === "h1" || tagName === "h2" || tagName === "h3") {
      ...
      debugLog("ODT Export - Found heading:", tagName)
  ...
  debugLog("ODT Export - Total paragraphs:", paragraphs.length)
  return paragraphs
}
...
// Use a font-face NAME (style:name) and a separate family string (svg:font-family / fo:font-family)
...
function generateContentXml(paragraphs: Paragraph[]): string {
  ...
  // Pre-scan to allocate style names deterministically
  ...
  // Character styles for inline marks
  ...
  // Paragraph styles (alignment) that inherit from Text body
...
export async function exportToOdt(editorElement: HTMLElement, filename = "document.odt"): Promise<void> {
  debugLog("ODT Export - Starting export")
  const paragraphs = parseEditorContent(editorElement)

  const zip = new JSZip()

  // ODT requires the mimetype file to be the first entry and uncompressed.
  ...
  debugLog("ODT Export - Export complete")
}
